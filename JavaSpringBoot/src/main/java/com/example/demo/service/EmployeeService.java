...
import com.example.demo.api.client.EmployeeFeignClient;
import com.example.demo.api.model.EmployeeDTO;
import com.example.demo.api.model.ResponseByIdDTO;
import com.example.demo.api.model.ResponseDTO;
...
public class EmployeeService{
	...
	public EmployeeService(EmployeeFeignClient client) {
		this.client = client;
	}


	public List<EmployeeDTO> getEmployees() throws JsonMappingException, JsonProcessingException{
		//This line is in case that the API response returns 429 Too Many Requests
		...
		//This is the line to consume the API if working
		//String res = client.getEmployees();
		ObjectMapper mapper = new ObjectMapper();
		ResponseDTO response = mapper.readValue(res, ResponseDTO.class);
		return response.getData();
	};
	
	public List<EmployeeDTO> getEmployeeById(int id) throws JsonMappingException, JsonProcessingException{
		//This line is in case that the API response returns 429 Too Many Requests
		String res = "{\"status\":\"success\","
				+ "\"data\":{\"id\":1,\"employee_name\":\"Tiger Nixon\",\"employee_salary\":320800,\"employee_age\":61,\"profile_image\":\"\"},"
		...
		List<EmployeeDTO> onePositionArray = new ArrayList<EmployeeDTO>();
		ObjectMapper mapper = new ObjectMapper();
		ResponseByIdDTO response= mapper.readValue(res, ResponseByIdDTO .class);
		onePositionArray.add(response.getData());
		return onePositionArray;
	}
}